...
This guide helps channel partners and system integrators integrate with OrbitLink Enterprise Gateway.

## Partner Types

- **Channel Partner** - Resell OrbitLink to customers
...
Contact OrbitLink admin team for partner account creation.

### 2. Authentication

```python
from orbitlink import OrbitLinkClient

client = OrbitLinkClient(base_url="https://api.orbitlink.com")
...
```json
{
  "webhook_url": "https://partner.com/webhooks/orbitlink",
...
client = OrbitLinkClient(api_key="partner_key")
networks = client.list_networks(organization="Customer Corp")
```

### REST API
```bash
curl "http://api.orbitlink.com/v1/telemetry/latest" \
  -H "Authorization: Bearer $TOKEN"
```
